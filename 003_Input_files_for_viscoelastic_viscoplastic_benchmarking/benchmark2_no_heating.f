...
! 
     ...
     5           B11=1475.0D0,B22=80.D0,B33=-3.2D0, PWRFPT=1.5D0, TQC=0.0D0)
!   ##################################################################################################################################
...
!   UPDATE DEFORMATION AND HEAT GENERATION
      ...
      DO M=1,NTENS
        ...
        RPL_VOL=RPL_VOL+(TQC*STRESS(M)*(DEPSILON_VP(M))/DTIME)
     ...
     1           (R2*((DEPSILON_VP(M))/DTIME)*((DEPSILON_VP(M))/DTIME))
      END DO
      DOT_VPSTRAN_2ND_INV=SQRT(P05*DOT_VPSTRAN_2ND_INV)
      ELSE
!   UPDATE DEFORMATION AND HEAT GENERATION
      RPL=R0    
      RPL_SHEAR=R0  
      ...
      DO M=1,NTENS
        ...
        RPL_VOL=RPL_VOL+(TQC*STRESS(M)*(DEPSILON_V(M))/DTIME)
...
!   ##################################################################################################################################
      ...
      VPSTRAN_2ND_INV=R0
      DO M=1,NDI
         VPSTRAN_1ST_INV=VPSTRAN_1ST_INV+(EPSILON_VP(M))
         VPSTRAN_2ND_INV=VPSTRAN_2ND_INV+(EPSILON_VP(M))*(EPSILON_VP(M))
      END DO                
      DO M=NDI+1,NTENS
        VPSTRAN_2ND_INV=VPSTRAN_2ND_INV+(R2*(EPSILON_VP(M))*(EPSILON_VP(M)))
      END DO
