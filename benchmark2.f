...
!   UPDATE DEFORMATION AND HEAT GENERATION
      ...
      DO M=1,NTENS
        ...
        RPL=RPL+(TQC*STRESS(M)*(DEPSILON_VP(M))/DTIME)
        RPL_SHEAR=RPL_SHEAR+(S(M)*DEPSILON_VP(M)/DTIME)
        RPL_VOL=RPL_VOL+(P*DEPSILON_VP(M)*SOID(M)/DTIME)
        VPSTRAN_1ST_INV=VPSTRAN_1ST_INV+EPSILON_VP(M)*SOID(M)
        DOT_VPSTRAN_1ST_INV=DOT_VPSTRAN_1ST_INV+(DEPSILON_VP(M))
     1                      /DTIME*SOID(M)
      END DO    
      DO M=1,NDI
         DOT_VPSTRAN_2ND_INV=DOT_VPSTRAN_2ND_INV+((DEPSILON_VP(M))/DTIME*(DEPSILON_VP(M))/DTIME)
         VPSTRAN_2ND_INV=VPSTRAN_2ND_INV+(EPSILON_VP(M))**R2
      END DO                
      DO M=NDI+1,NTENS
        DOT_VPSTRAN_2ND_INV=DOT_VPSTRAN_2ND_INV+(R2*((DEPSILON_VP(M))/DTIME*(DEPSILON_VP(M))/DTIME))
...
!   UPDATE DEFORMATION AND HEAT GENERATION
      ...
      DO M=1,NTENS
        ...
        RPL=RPL+(TQC*STRESS(M)*(DEPSILON_V(M))/DTIME)
        RPL_SHEAR=RPL_SHEAR+(S(M)*DEPSILON_V(M)/DTIME)
        RPL_VOL=RPL_VOL+(P*DEPSILON_V(M)*SOID(M)/DTIME)
        VPSTRAN_1ST_INV=VPSTRAN_1ST_INV+EPSILON_VP(M)*SOID(M)
        DOT_VPSTRAN_1ST_INV=DOT_VPSTRAN_1ST_INV+(DEPSILON_V(M))
     1                      /DTIME*SOID(M)
      END DO    
      DO M=1,NDI
         DOT_VPSTRAN_2ND_INV=DOT_VPSTRAN_2ND_INV+((DEPSILON_V(M))/DTIME*(DEPSILON_V(M))/DTIME)
         VPSTRAN_2ND_INV=VPSTRAN_2ND_INV+(EPSILON_VP(M))*(EPSILON_VP(M))
      END DO
      DO M=NDI+1,NTENS
        DOT_VPSTRAN_2ND_INV=DOT_VPSTRAN_2ND_INV+(R2*((DEPSILON_V(M))/DTIME)*((DEPSILON_V(M))/DTIME))
...
!      COMPUTE FRACTION OF MELT AT TIME STEP       
       ...
       ELSE
          FPT=R0
       END IF  
       IF(DOT_VPSTRAN_2ND_INV.EQ.R0)THEN
          LOG10_DOT_VPSTRAN_2ND_INV=R0
       ELSE
          LOG10_DOT_VPSTRAN_2ND_INV=LOG10(DOT_VPSTRAN_2ND_INV)
       END IF
!     UPDATE STATE VARIABLES
      ...
      STATEV(NTENS+8)=LOG10(DOT_VPSTRAN_2ND_INV)       !     SDV 12    ! LOG10 OF THE SECOND INVARIANT OF VISCOPLASTIC STRAIN RATE 
      ...
      STATEV(NTENS+24)=DGAMA_VISC                             !     SDV 28    ! FRACTION OF MELT      
      RETURN
      END      
!   ##################################################################################################################################
!   ##################################################################################################################################
