...
!   ################################################################################################################################## 
        ...
        H=PROPS(4)                        		  !     HARDENING MODULUS (IF HARDENING IS USED. IT IS NOT USED HERE)
        PWR=PROPS(5)                        	        !     STRESS EXPONENT 
        phi_i=PROPS(6)                      	        !     INITIAL FRICTION ANGLE (NOT USED HERE)
        phi_f=PROPS(7)      				        !     FINAL FRICTION ANGLE   (IF FRICTIONAL HARDENING OR SOFTENING IS USED. IT IS CONSTANT IF NOT) 
        ...
        TREF=PROPS(15)					        !     REFERENCE TEMPERATURE (FROM OLD VERSION OF CODE, NOT USED HERE)
        ALPH_LE=PROPS(16)					  !     COEFFICIENT OF LINEAR EXPANSION (FROM OLD VERSION OF CODE, NOT USED HERE)
        EPSILON_BAR_C=PROPS(17)                         !     CRITICAL EFFECTIVE PLASTIC STRAIN (OLD VERSION OF THE CODE, NOT USED HERE)
!   ##################################################################################################################################
...
!   UPDATE DEFORMATION AND HEAT GENERATION
      ...
      DO M=1,NTENS
        ...
        RPL=RPL+(S(M)*(DEPSILON_VP(M))/DTIME)
...
!      VPSTRAN_2ND_INV=R0
      ...
      DO M=1,NTENS
        ...
        RPL=RPL+(VSTRESS_DEV(M)*(DEPSILON_V(M))/DTIME)
!        VPSTRAN_1ST_INV=VPSTRAN_1ST_INV+EPSILON_VP(M)*SOID(M)
...
!     UPDATE STATE VARIABLES
      ...
      DO I=1,NTENS
        STATEV(I)=EPSILON_VP(I)                        !     SDV 1-4   ! VISCOUS + PLASTIC STRAINS (11,22,33,12)
      END DO 
      STATEV(NTENS+1)=GAMMA_BAR                        !     SDV 5     ! HARDENING HISTORY (IF COHESION HARDENING IS USED)
      STATEV(NTENS+2)=PHI_YDP       	             !     SDV 6     ! DRUCKER-PRAGER YIELD CRITERION
      STATEV(NTENS+3)=SQRTJ2                           !     SDV 7     ! INITIAL J2
      STATEV(NTENS+4)=SQRTJ2T		                   !     SDV 8     ! FINAL J2
      ...
      STATEV(NTENS+11)=VISCOSITY                       !     SDV 15    ! EFFECTIVE VISCOSITY     
      ...
      STATEV(NTENS+16)=DEPSILON_V(1)/DTIME             !     SDV 20    ! VISCOUS STRAIN RATE(11)
      STATEV(NTENS+17)=DEPSILON_V(2)/DTIME             !     SDV 21    ! VISCOUS STRAIN RATE(22)
      STATEV(NTENS+18)=DEPSILON_V(3)/DTIME             !     SDV 22    ! VISCOUS STRAIN RATE(33)
      STATEV(NTENS+19)=DEPSILON_V(4)/DTIME             !     SDV 23    ! VISCOUS STRAIN RATE(12)
      STATEV(NTENS+20)=DEPSILON_VP(1)/DTIME            !     SDV 24    ! VISCOPLASTIC STRAIN RATE(11)
      STATEV(NTENS+21)=DEPSILON_VP(2)/DTIME            !     SDV 25    ! VISCOPLASTIC STRAIN RATE(22)
      STATEV(NTENS+22)=DEPSILON_VP(3)/DTIME            !     SDV 26    ! VISCOPLASTIC STRAIN RATE(33)
      STATEV(NTENS+23)=DEPSILON_VP(4)/DTIME            !     SDV 27    ! VISCOPLASTIC STRAIN RATE(12)
